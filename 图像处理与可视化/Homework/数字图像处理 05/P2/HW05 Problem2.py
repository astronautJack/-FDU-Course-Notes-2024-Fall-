        ...
        for center, cutoff in zip(centers[i], cutoffs[i]):
            if filter_type == -2:  # Line filter (horizontal or vertical)
                if center[3] == 0:  # Horizontal line
                    ...
                    x_start, x_end = np.clip(center[1], 0, N), np.clip(center[2], 0, N-1)
                    mask_x_range = np.arange(x_start, x_end + 1)
                    mask_y_range = np.arange(np.clip(y_val - cutoff, 0, M), np.clip(y_val + cutoff, 0, M-1) + 1)
                    ...
                    y_start, y_end = np.clip(center[1], 0, M), np.clip(center[2], 0, M-1)
                    mask_y_range = np.arange(y_start, y_end + 1)
                    mask_x_range = np.arange(np.clip(x_val - cutoff, 0, N), np.clip(x_val + cutoff, 0, N-1) + 1)
                    mask = list(itertools.product(mask_y_range, mask_x_range))
                    mask += [(M - i - 1, N - j - 1) for i, j in mask]
...
if __name__ == "__main__":
    ...
    if option == 1:
        ...
        cutoffs_1 = [5] * len(centers_1)  # Set a 20-pixel cutoff for all centers_1 entries
        ...
        cutoffs_2 = [10] * len(centers_2)  # Set 30-pixel cutoff for all centers_2 entries
