...
function [Q, R] = Gram_Schmidt_QR(A, tolerance, modified, reorthogonalized)
    ...
    r = n;  % Initialize rank of A, assuming full rank initially
    ...
    for k = 1:n
        ...
        if modified
            for iter = 1:max_iter  % Repeat orthogonalization based on max_iter
                for i = 1:k-1  % Loop over previously computed columns of Q
                    delta(i) = Q(1:m, i)' * Q(1:m, k);  % Compute projection of Q_k on Q_i
                    R(i, k) = R(i, k) + delta(i);  % Update the corresponding entry in R
                    Q(1:m, k) = Q(1:m, k) - delta(i) * Q(1:m, i);  % Subtract projection from Q_k
                end
            end
        else  % Classical Gram-Schmidt (CGS)
            for iter = 1:max_iter
                delta(1:k-1) = Q(1:m, 1:k-1)' * Q(1:m,k);  % Compute projections in one step
                R(1:k-1, k) = R(1:k-1, k) + delta(1:k-1);  % Update R
                Q(1:m, k) = Q(1:m, k) - Q(1:m, 1:k-1) * delta(1:k-1);  % Subtract the projection from Q_k
            end
        end
        ...
        if R(k, k) < tolerance
            r = k - 1;  % Adjust the rank of A, since this column is linearly dependent
            break;  % Stop the process as further columns will also be dependent
        end
    ...
    % Additional step: if the number of columns n is greater than m + 2,
